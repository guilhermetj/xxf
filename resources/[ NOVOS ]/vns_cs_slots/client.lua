...
func = Tunnel.getInterface("vns_cs_slots")
funcc = {}
		...
		if state then
			...
			Citizen.CreateThread(function()
				while selectedSlot ~= nil do
					...
					if IsDisabledControlJustPressed(0, 194) and not self.running then	-- BACKSPACE
                    ...
                    if DoesEntityExist(v.tableObject) then
                        ...
                        if dist < 2.0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                TriggerServerEvent('server_remote:slots:taskSitDown', k, v.index, closestChairData)
                            end
                            break
                        end
                    end
...
RegisterNetEvent('client_callback:slots:taskSitDown')
AddEventHandler(
    'client_callback:slots:taskSitDown',
    function(k, index, closestChairData)
		ACTIVE_SLOT = index
