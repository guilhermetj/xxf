	...
	if user_id then
		if stockadePlates[vehPlate] == nil then
			if vRP.getInventoryItemAmount(user_id,stockadeItem) >= 1 then
				...
				TriggerClientEvent("Notify",source,"roxo","Você não possui um <b>"..vRP.itemNameList(stockadeItem).."</b>.",5000)
			end
		else
			if stockadePlates[vehPlate] > 0 then
				...
				vRP.giveInventoryItem(user_id,"dollars2",math.random(10000,18000),true)
			else
