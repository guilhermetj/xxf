...
function cnVRP.paymentMethod()
	...
	if user_id then
		...
		local value = math.random(700,1300)
...
RegisterCommand("tow",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasPermission(user_id,"Mechanic") or vCLIENT.checkService(source) then
			if vRPclient.getHealth(source) > 101 then
				vCLIENT.towPlayer(source)
				userList[user_id] = source
			end
		else
			TriggerClientEvent("Notify",source,"roxo","Somente mecanicos ou trabalhadores do reboque podem utilizar deste serviço.",5000)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
