		...
		if vRP.hasPermission(user_id,"TheLost") then
			if parseInt(gallons[processId][5]) <= 0 then
				if vRP.tryGetInventoryItem(user_id,"analgesic",parseInt(gallons[processId][4])) then
					...
					TriggerClientEvent("Notify",source,"amarelo","Você precisa de <b>"..vRP.format(parseInt(gallons[processId][4])).."x "..vRP.itemNameList("analgesic").."</b>.",5000)
				end
	...
	if user_id then
		if vRP.hasPermission(user_id,"TheLost") then
			if vRP.tryGetInventoryItem(user_id,"methliquid",10) then
				...
				TriggerClientEvent("Notify",source,"amarelo","Você precisa de <b>10x "..vRP.itemNameList("methliquid").."</b>.",5000)
			end
		end
	end
end
