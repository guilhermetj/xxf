...
-----------------------------------------------------------------------------------------------------------------------------------------
 ...
 --[[RegisterCommand("bvida",function(source,args,rawCommand)
	...
	local velocidade = GetEntityVelocity(ped)
	Citizen.Wait(50)
	local velocidade2 = GetEntityVelocity(ped)
	if velocidade == velocidade2 then
        TriggerServerEvent("vrp_admin:bvida")
        print(1)
        local model = GetEntityModel(ped)
        SetPlayerModel(PlayerId(),model)
	else
		TriggerEvent("Notify","roxo","Fique parado para usar o <b>/bvida</b>",10000)
	end
end) ]]--
-----------------------------------------------------------------------------------------------------------------------------------------
-- TELEPORTWAY
...
RegisterNetEvent("bvidaFixVida")
AddEventHandler("bvidaFixVida",function(vida)
...
--RegisterKeyMapping("nc","Admin: Noclip","keyboard","o") -- FUNCAO DE NOCLIP NA LETRA O # RETIREI  PRA NAO USAR SEM QUERER
-----------------------------------------------------------------------------------------------------------------------------------------
-- VEHICLETUNING
-----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
  ...
  -- NPC CONTROL
-----------------------------------------------------------------------------------------------------------------------------------------
trafficDensity = 0.0  -- 0.1  aqui voce coooloca a quantidade de npc CARROS
pedDensity = 0.0  -- 0.5 aqui voce coooloca a quantidade de npc APE
Citizen.CreateThread(function()
 AddTextEntry("FE_THDR_GTAO","Midnight Community - Tenha um bom RP")
   while true do
        ...
        SetCreateRandomCopsNotOnScenarios(true) --Para o Spanw Aleatorio de Policias Fora do Cenario
        SetGarbageTrucks(true) --Desactiva os Camioes do Lixo de dar Spawn Aleatoriamente
        SetCreateRandomCopsOnScenarios(true) --Para o Spanw Aleatorio de Policias no Cenario
