...
function updateDisplayHud()
	...
	if IsPedInAnyVehicle(ped) then
		...
		SendNUIMessage({ vehicle = true, talking = talking, health = health, armour = armour, thirst = thirst, hunger = hunger, stress = stress, street = streetName, direction = flexDirection, radio = radioDisplay, hours = hours, minutes = minutes, voice = voice, oxigen = oxigen, fuel = fuel, speed = speed, showbelt = showBelt, seatbelt = beltLock })
	else
		SendNUIMessage({ vehicle = false, talking = talking, health = health, armour = armour, thirst = thirst, hunger = hunger, stress = stress, street = streetName, direction = flexDirection, radio = radioDisplay, hours = hours, minutes = minutes, voice = voice, oxigen = oxigen })
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD
			...
			if not IsPedOnAnyBike(ped) and not IsPedInAnyHeli(ped) and not IsPedInAnyPlane(ped) then
				...
				if GetPedInVehicleSeat(vehicle,-1) == ped then
					...
					if speed ~= beltSpeed then
						...
						if ((beltSpeed - speed) >= 35 and beltLock == 0) then
