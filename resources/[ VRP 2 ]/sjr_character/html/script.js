    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            if (data.toggle) {
                ...
                $.post('http://sjr_character/setupCharacters');
                setTimeout(function(){
                    saMultiCharacters.fadeInDown('.characters-list', '20%', 400);
                    $.post('http://sjr_character/removeBlur');
                }, 2000);
            } else {
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('http://sjr_character/closeUI');
    $.post('http://sjr_character/disconnectButton');
});
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('http://sjr_character/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
$(document).on('click', '#create', function(e){
    e.preventDefault();
    if (true) {
        $.post('http://sjr_character/createNewCharacter', JSON.stringify({
        ...
        console.log('[sjr_character] Tried to create an char with non-english name.')
    }
});

$(document).on('click', '#accept-delete', function(e){
    $.post('http://sjr_character/removeCharacter', JSON.stringify({
...
$("#close-reg").click(function (e) {
    ...
    $('.characters-list').css("filter", "none")
    saMultiCharacters.fadeOutDown('.character-register', '125%', 400);
    $.post('http://sjr_character/getOffChar');
...
function select_character(id) {
    var chari = id
    $.post('http://sjr_character/getCloserToCharacter', JSON.stringify({
        charid: chari
    }));

    //$.post('http://sjr_character/selectCharacter', JSON.stringify({}));
...
function update_char_marker(id) {
    var chari = id
    $.post('http://sjr_character/updateCharMarker', JSON.stringify({
        charid: chari
    }));
};
