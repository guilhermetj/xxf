...
local garages = {
	...
	[34] = { ["name"] = "Garage", ["payment"] = false, ["public"] = true },
	...
	[760] = { ["name"] = "Tow", ["payment"] = false, ["public"] = true },
	...
	[762] = { ["name"] = "Garage", ["payment"] = false, ["public"] = true },
	[763] = { ["name"] = "towtruck", ["payment"] = false, ["perm"] = Mechanic },
	[764] = { ["name"] = "flatbed", ["payment"] = false, ["perm"] = Mechanic },
	[765] = { ["name"] = "amarokpolicia", ["payment"] = false, ["perm"] = Police },
	[766] = { ["name"] = "cb500policia2", ["payment"] = false, ["perm"] = Police },
	[767] = { ["name"] = "fk8", ["payment"] = false, ["perm"] = Mechanic },
	[768] = { ["name"] = "pbike", ["payment"] = false, ["perm"] = Mechanic },
	[769] = { ["name"] = "Garage", ["payment"] = false, ["public"] = true },
	[770] = { ["name"] = "Garage", ["payment"] = false, ["public"] = true },
	[771] = { ["name"] = "Minerio", ["payment"] = false, ["public"] = true },
	[772] = { ["name"] = "Caminhoneiro", ["payment"] = false, ["public"] = true },
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- GARAGES
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- SPAWNVEHICLES
-----------------------------------------------------------------------------------------------------------------------------------------
function cnVRP.spawnVehicles(name,use,openGarage)
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id and name then
        if not vCLIENT.returnVehicle(source,name) then
			...
			if parseInt(os.time()) <= parseInt(vehicle[1].time+24*60*60) then
				TriggerClientEvent("Notify",source,"vermelho","Veiculo com taxa atrasada.",5000)
			elseif parseInt(vehicle[1].arrest) >= 1 then
				TriggerClientEvent("Notify",source,"vermelho","Veiculo detido.",5000)
				...
				if garages[use].payment and not vRP.getPremium(parseInt(user_id)) then
					if vRP.getBank(parseInt(user_id)) >= parseInt(vRP.vehiclePrice(name)*0.05) then
						...
						if status and vRP.paymentBank(parseInt(user_id),parseInt(vRP.vehiclePrice(name)*0.05)) then
							...
							if openGarage == "Police" then
								print(openGarage.. user_id..vRP.getUserIdentity(user_id).name)
							end
					...
					if status then
						...
						spanwedVehs[name..user_id] = true
						if openGarage == "Police" then
							print(openGarage.. user_id..vRP.getUserIdentity(user_id).name..name)
						end
						TriggerEvent("setPlateEveryone",vehicle[1].plate)
					end
				end
...
RegisterCommand("dv",function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"Admin") then
		local vehicle = vRPclient.getNearVehicle(source,15)
		if vehicle then
			local vehname = vRPclient.getModelName(source,vehicle)
			vCLIENT.deleteVehicle(source,vehicle)
...
AddEventHandler("vRP:playerLeave",function(user_id,source)
	...
	for k,v in pairs(vehicle) do
		spanwedVehs[v.vehicle..user_id] = nil
		vCLIENT.syncNameDelete(source,v.vehicle)
		Citizen.Wait(1)
	end
end)
		...
		if vehicle and vehPlate then
			...
			if user_id == plateUserId or stealVehs[vehPlate] == user_id then
				...
				if vehLock == 1 then
					...
					TriggerClientEvent("Notify",source,"unlocked","Veículo <b>destrancado</b> com sucesso.",5000)
				else
					TriggerClientEvent("vrp_sound:source",source,"lock",0.3)
					TriggerClientEvent("Notify",source,"locked","Veículo <b>trancado</b> com sucesso.",5000)
				end
...
-- VEHICLELOCK
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("vrp_garages:vehiclelock")
AddEventHandler("vrp_garages:vehiclelock",function()
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id then
        local vehicle,vehNet,vehPlate,vehName,vehLock = vRPclient.vehList(source,11)
...
RegisterCommand("vehs",function(source,args,rawCommand)
	...
	if user_id then
		if args[1] == "transfer" and parseInt(args[3]) > 0 then
			...
			if myvehicles[1] then
				...
				if vRP.getPremium(parseInt(args[3])) then
					if parseInt(maxVehs[1].qtd) >= parseInt(myGarages[1].garage) + 2 then
						TriggerClientEvent("Notify",source,"roxo","Você atingiu o número máximo de veículos na garagem.",5000)
						return
					end
				else
			...
			local vehicle = vRP.query("vRP/get_vehicle",{ user_id = parseInt(user_id) })
			for k,v in ipairs(vehicle) do
				TriggerClientEvent("Notify",source,"roxo","<b>Modelo:</b> "..vRP.vehicleName(v.vehicle).." ( "..v.vehicle.." )",20000)
				Citizen.Wait(1)
			end
		end
	end
