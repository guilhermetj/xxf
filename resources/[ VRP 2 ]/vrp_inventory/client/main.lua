...
RegisterNUICallback("requestMochila",function(data,cb)
	...
	for k,v in pairs(droplist) do
		...
		if GetDistanceBetweenCoords(v.x,v.y,cdz,x,y,z,true) <= 1.5 then
			table.insert(dropItems,{ name = v.name, key = v.name, amount = v.count, index = v.index, peso = v.peso, desc = v.desc, id = k, tipo = v.tipo, economy = v.economy })
		end
	end
...
-- REPARAR MOTOR
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("vrp_inventory:repararMotor")
AddEventHandler("vrp_inventory:repararMotor",function(index,status)
	if NetworkDoesNetworkIdExist(index) then
		local v = NetToEnt(index)
		if DoesEntityExist(v) then
			local fuel = GetVehicleFuelLevel(v)
...
AddEventHandler("vrp_inventory:repairTires",function(index)
	...
	if NetworkDoesNetworkIdExist(index) then
		local v = NetToEnt(index)
		if DoesEntityExist(v) then
			--[[ local pneuMaisProximo = GetClosestVehicleTire(v)
			print(pneuMaisProximo.tireIndex)
			if IsVehicleTyreBurst(v, pneuMaisProximo.tireIndex, 0) then
				if pneuMaisProximo ~= nil then
					TriggerEvent("Notify","verde","Pneu reparado",5000)
					TriggerServerEvent("vrp_inventory:syncPneuConcertarS",index,pneuMaisProximo.tireIndex)
				else
					TriggerEvent("Notify","verde","Nenhum pneu encontrado",5000)
					TriggerServerEvent("vrp_inventory:giveTireBack")
				end
			else
				TriggerEvent("Notify","verde","O pneu não está furado",5000)
				TriggerServerEvent("vrp_inventory:giveTireBack")
			end ]]
			for i = 0,8 do
...
function func.removeWeaponInHand2()
...
RegisterNetEvent("vrp_inventory:syncPneuConcertarC")
AddEventHandler("vrp_inventory:syncPneuConcertarC",function(vehicle,tireIndex)
	SetVehicleTyreFixed(NetToEnt(vehicle),tyreIndex)
end)
...
function func.fishingStatus()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	local distance = #(coords - vector3(-920.68,5728.79,1.09))
	if distance <= 65 then
		return true
	end
	return false
...
function func.putWeaponHands(weapon)
    -- if not takingWeapon then
        ...
        if not HasPedGotWeapon(ped, GetHashKey(currentWeaponModel), false) then
            ...
            RemoveWeaponFromPed(ped, GetHashKey(currentWeaponModel))
			Wait(10)
			--[[ GiveWeaponToPed(ped, wep, 0, false, false) ]]
            currentWeapon = nil
            currentWeaponModel = nil
        --end
     end
...
function GetClosestVehicleTire(vehicle)
	...
	for a = 1, #tireBones do
		local tirebones = tireBones[a]
		bonePos = GetWorldPositionOfEntityBone(vehicle, GetEntityBoneIndexByName(vehicle, tirebones))
		local distance = Vdist(plyPos.x, plyPos.y, plyPos.z, bonePos.x, bonePos.y, bonePos.z)

		if closestTire == nil then
			if distance <= minDistance then
				local tirebones = tireBones[a]
				closestTire = {bone = tirebones, boneDist = distance, bonePos = bonePos, tireIndex = tireIndex[tirebones]}
				break
			end
		end
	end
