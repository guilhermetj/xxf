...
RegisterCommand('createChest',function(source, args, rawCommand)
	...
	if user_id and vRP.hasPermission(user_id,"Owner") then
		...
		local nome = vRP.prompt(source,"NOME DO BAU","")
		if nome == "" then
			return
		end

		local perm = vRP.prompt(source,"PERMISSAO DO BAU","")
		if perm == "" then
			return
		end

		local tamanho = vRP.prompt(source,"TAMANHO DO BAU","")
		if tamanho == "" then
			return
		end
	...
	if itemName then
		...
		if user_id then
			if noStore[itemName] then
				TriggerClientEvent("Notify",source,"roxo","Você não pode armazenar este item em baús.",5000)
				return
			end
			...
			if consult[1].name == tostring(chestOpen[parseInt(user_id)]) then
				if vRP.storeChestItem(user_id,"chest:"..tostring(chestOpen[parseInt(user_id)]),itemName,amount,consult[1].weight,slot,'chest') then
					TriggerClientEvent("vrp_chest:Update",source,"updateChest")
				end
			end
		end
