...
RegisterCommand('travartime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            freezeTime = not freezeTime
            if freezeTime then
                TriggerClientEvent('vSync:notify', source, 'Tempo esta ~b~congelado~s~.')
            else
                TriggerClientEvent('vSync:notify', source, 'Tempo não esta ~y~congelado~s~.')
            end
        end
    else
        freezeTime = not freezeTime
...
RegisterCommand('travarclima', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
