...
RegisterCommand("rprender",function(source,args,rawCommand)
	...
	if user_id then
		if vRP.hasPermission(user_id,"Police") then
			...
			local services = vRP.prompt(source,"TEMPO A SER REDUZIDO","")
			if services == "" then
				return
			end
...
function CNVcore.saveItems()
    ...
    if user_id then
		...
		TriggerClientEvent("Notify",source,"roxo","Você poderá retirar seus pertences ná recepção assim que for liberado.",5000)
	end
end
...
AddEventHandler("vRP:playerSpawn",function(user_id,source)
	...
	if parseInt(consult[1].prison) <= 0 then
		...
		TriggerClientEvent("Notify",source,"roxo","Você ainda tem que cumprir <b>"..parseInt(consult[1].prison).." serviços</b>.",5000)
...
-- 				TriggerClientEvent("Notify",source,"roxo","Multa aplicada em <b>"..identity.name.." "..identity.name2.."</b> no valor de <b>$"..vRP.format(parseInt(value)).." dólares</b>.",5000)
