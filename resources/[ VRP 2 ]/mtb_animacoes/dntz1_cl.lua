...
local x,y,z,h
local poledance = {
	{['x'] = 108.97, ['y'] = -1289.28, ['z'] = 28.25, ['h'] = 297.89},
	{['x'] = 105.04, ['y'] = -1294.39, ['z'] = 28.25, ['h'] = 297.89},
	{['x'] = 102.48, ['y'] = -1290.07, ['z'] = 28.25, ['h'] = 297.89},
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for k,v in pairs(poledance) do
			local distance = GetDistanceBetweenCoords(coord,v.x,v.y,v.z)
			if distance < 2 then
				sleep = 5
				DrawText3Ds(v.x,v.y,v.z+1,"~r~E ~w~ Poledance")
				if IsControlJustPressed(0,38) then
					ToggleActionMenu()
					x = v.x
					y = v.y
...
local dances = {
	{nomeAnimacao = "poledance1",dict = "mini@strip_club@pole_dance@pole_dance1",anim = "pd_dance_01"},
	{nomeAnimacao = "poledance2",dict = "mini@strip_club@pole_dance@pole_dance2",anim = "pd_dance_02"},
	{nomeAnimacao = "poledance3",dict = "mini@strip_club@pole_dance@pole_dance3",anim = "pd_dance_03"},
}

function PlayAnim(anim)
	for k,v in pairs(dances) do
		if v.nomeAnimacao == anim then
			RequestAnimDict(v.dict)
			while not HasAnimDictLoaded(v.dict) do
				Wait(0)
			end
			vRP.playAnim(false,{v.dict,v.anim},true)
