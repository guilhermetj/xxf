...
RegisterCommand("enter",function(source,args)
	...
	for k,v in pairs(homesList) do
		...
		if distance <= 1.5 and vSERVER.checkPermissions(k) then
			...
			if v[1] == "Mansion" then
				createMansion(ped,v[5],v[6],1503.50)
			end

			if v[1] == "Favela" then
				createFavela(ped,v[5],v[6],1500.0)
			end
			...
			if v[1] == "Mansion" then
				SetEntityCoords(ped,v[5]+10.0,v[6]+7.0,1508.50)
				table.insert(internHouses,{ v[5]+10.0,v[6]+7.0,1509.00,"exit","SAIR" })
				table.insert(internHouses,{ v[5]-6.70,v[6]+6.50,1508.00,"vault","ABRIR" })
			end

			if v[1] == "Favela" then
				SetEntityCoords(ped,v[5]-3.20,v[6]-3.50,1498.0)
				table.insert(internHouses,{ v[5]-3.20,v[6]-3.50,1500.0,"exit","SAIR" })
				table.insert(internHouses,{ v[5]+0.10,v[6]-3.50,1500.0,"vault","ABRIR" })
			end
...
RegisterCommand("invadir",function(source,args)
	...
	for k,v in pairs(homesList) do
		...
		if distance <= 1.5 and vSERVER.checkPolice() then
			...
			if v[1] == "Mansion" then
				createMansion(ped,v[5],v[6],1500.0)
			end
			...
			if v[1] == "Mansion" then
                SetEntityCoords(ped,v[5]+11.0,v[6]+7.80,1510.0)
				table.insert(internHouses,{ v[5]+11.10,v[6]+7.80,1510.50,"exit","SAIR" })
				table.insert(internHouses,{ v[5]-6.0,v[6]+7.0,1509.30,"vault","ABRIR" })
			end

			if v[1] == "Favela" then
				SetEntityCoords(ped,v[5]-2.40,v[6]-3.50,1501.0)
				table.insert(internHouses,{ v[5]-2.40,v[6]-3.50,1501.0,"exit","SAIR" })
				table.insert(internHouses,{ v[5]+0.80,v[6]-3.50,1500.7,"vault","ABRIR" })
			end
...
RegisterCommand("casas",function(source,args)
	...
	if casas then
		TriggerEvent("Notify","verde","Adicionado a marcação das residências.",3000)
		for k,v in pairs(homesList) do
		...
		TriggerEvent("Notify","verde","Removido a marcação das residências.",3000)
...
function createMansion(ped,x,y,z)
	homeObjects[1] = CreateObject(GetHashKey("shell_franklin_01"),x-0.7,y-0.4,z-1.42,false,false,false)
...
--- FAVELA
-----------------------------------------------------------------------------------------------------------------------------------------
function createFavela(ped,x,y,z)
	homeObjects[1] = CreateObject(GetHashKey("favela_shell_01"),x-0.7,y-0.4,z-1.42,false,false,false)
