...
function gcp.checkCelular()
	...
	if user_id then
		if vRP.getInventoryItemAmount(user_id,"cellphone") >= 1 then
			...
			TriggerClientEvent("Notify",source,"Você não possuí um <b>Celular</b>.")
			return false
		end
	end
end
...
AddEventHandler("vRP:playerSpawn",function(user_id,source,first_spawn)
	if first_spawn then
		...
		getOrGeneratePhoneNumber(sourcePlayer,identifier,function(myPhoneNumber)
			...
			TriggerClientEvent("gcPhone:contactList",sourcePlayer,getContacts(identifier))
			TriggerClientEvent("gcPhone:requestAllMessages",sourcePlayer,requestAllMessages(identifier))
			TriggerClientEvent("gcPhone:allMessage",sourcePlayer,getMessages(identifier))
			TriggerClientEvent("gcPhone:allUpdate", sourcePlayer)
		end)
	end
end)
...
AddEventHandler('gcPhone:allUpdate',function()
	...
	local money = vRP.getBank(sourcePlayer)
	TriggerClientEvent("gcPhone:myPhoneNumber",sourcePlayer,num)
	TriggerClientEvent("gcPhone:contactList",sourcePlayer,getContacts(identifier))
	TriggerClientEvent("gcPhone:allMessage",sourcePlayer,getMessages(identifier))
	sendHistoriqueCall(sourcePlayer,num)
    TriggerClientEvent("bank:Update", sourcePlayer, money)
end)
...
AddEventHandler("vRP/update_gc_phone", function()
    if source ~= nil then
        ...
        if user_id ~= nil then
            ...
            TriggerClientEvent("bank:Update", source, bmoney)
        end
    end
end)
...
local webhookbanco = "https://discord.com/api/webhooks/864314528996655124/OFdD-JFtL9miVOv0SKRgZlZsTeqf_3aYIl6IvavVst7jMhmZ_QPSVcIa8bWGqirbcJHA"
...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
    if user_id ~= nil then
        ...
        TriggerClientEvent("bank:Update", source, money)
    end
end)
...
AddEventHandler('bank:transfer', function(id, amount)
    ...
    local heId = vRP.getUserId(targetPlayer)
    local amount =  tonumber(amount)
    if amount <= 0 then
        return TriggerClientEvent("Notify",_source,"vermelho","Você digitou uma quantia inválida.", 5000)
    end
    local identity = vRP.getUserIdentity(user_id)
    local identityT = vRP.getUserIdentity(tonumber(id))    
    if targetPlayer == nil then
        return TriggerClientEvent("Notify",_source,"vermelho","Passaporte inválido.", 5000)
    else
        local myBank = vRP.getBank(user_id)
        local heBank = vRP.getBank(heId)
        if tonumber(user_id) ~= tonumber(id) then
            if myBank >= parseInt(amount) then
                vRP.setBank(user_id,myBank - amount)
                vRP.setBank(heId,heBank + amount)
                --TriggerClientEvent("Notify",targetPlayer,"sucesso","Você recebeu $" .. amount .." de "  ..identity.name.. " ID: " .. tostring(user_id), 5000)
                TriggerClientEvent("Notify", targetPlayer, "sucesso", "Você recebeu R$"..amount.." de "..identity.name.. " "..identity.name2.." | [".. user_id.."]", 5000)
                --TriggerClientEvent("Notify", _source, "sucesso", "Você recebeu R$"..amount.." de "..identity.name.. " "..identity.name2.." [".. user_id.."]", 5000)
                --TriggerClientEvent("Notify",_source,"sucesso","Tranferencia sucedida")       
                TriggerClientEvent("Notify", _source, "sucesso", "Você enviou R$"..amount.." para "..identityT.name.. " "..identityT.name2.. " | ["..heId.."]", 5000)
                ...
                TriggerClientEvent("Notify",_source,"sucesso","Você não possui saldo suficiente.") 
            end
        end
    end
end)
